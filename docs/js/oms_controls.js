...
var megaDelta = {
	...
	"Hearthflame Mask": {
		...
		ability: "Mold Breaker",
		type: "Fire",
		skip: ["Ogerpon-Hearthflame", "Ogerpon-Hearthflame-Tera"],
	},
	"Wellspring Mask": {
		at: 0,
		df: 0,
		...
		type: "Water",
		skip: ["Ogerpon-Wellspring", "Ogerpon-Wellspring-Tera"],
	},
	"Cornerstone Mask": {
		at: 0,
		df: 0,
		...
		ability: "Sturdy",
		type: "Rock",
		skip: ["Ogerpon-Cornerstone", "Ogerpon-Cornerstone-Tera"],
	},
};
...
function autoUpdateStats(p) {
	...
	if (!(shouldUseTS || shouldUseScale || shouldUseMnM)) {
		...
		var abilityFallback =
			typeof pokemon.abilities !== "undefined" ? pokemon.abilities[0] : "";
	...
	if (shouldUseMnM) {
		...
		if (megaDelta[item] && megaDelta[item].skip.indexOf(pokemonName) < 0) {
			...
			for (var i = 1; i < LEGACY_STATS[gen].length; i++) {
				...
				volatileCopy.bs[stat] = clampStats(
					pokemon.bs[stat] + megaDelta[item][stat]
				);
	...
	if (shouldUseTS) {
		...
		var tierAddon =
			(gen >= 9 ? CONVERSION_G9 : CONVERSION_PREG9)[tier.toLowerCase()] || 0;
		for (var i = 1; i < LEGACY_STATS[gen].length; i++) {
			var stat = LEGACY_STATS[gen][i];
