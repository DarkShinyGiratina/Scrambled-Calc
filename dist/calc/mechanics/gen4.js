...
function calculateDPP(gen, attacker, defender, move, field) {
    ...
    (0, util_1.checkInstinct)(attacker, defender);
    (0, util_1.checkInstinct)(defender, attacker);
    ...
    if (secondDefenderType && firstDefenderType !== secondDefenderType) {
        ...
        if (firstTypePrecedence > secondTypePrecedence) {
            _a = __read([
                secondDefenderType,
                firstDefenderType,
            ], 2), firstDefenderType = _a[0], secondDefenderType = _a[1];
        }
    }
    ...
    if ((move.dropsStats && move.timesUsed > 1) || move.hits > 1) {
        ...
        var _loop_1 = function (times) {
            ...
            var baseDamage_1 = Math.floor(Math.floor((Math.floor((2 * attacker.level) / 5 + 2) * newBasePower * newAtk) /
                50) / defense);
            if (attacker.hasStatus("brn") &&
                isPhysical &&
...
function calculateDefenseDPP(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    if (defender.hasItem("Soul Dew") &&
        defender.named("Latios", "Latias") &&
        !isPhysical) {
        defense = Math.floor(defense * 1.5);
        desc.defenderItem = defender.item;
    }
    else if ((defender.hasItem("Deep Sea Scale") &&
...
function calculateFinalModsDPP(baseDamage, attacker, move, field, desc, isCritical) {
    ...
    if (attacker.hasAbility("Flash Fire") &&
        attacker.abilityOn &&
        move.hasType("Fire")) {
        baseDamage = Math.floor(baseDamage * 1.5);
