...
function display(gen, attacker, defender, move, field, damage, rawDesc, notation, err) {
    ...
    return koChanceText
        ? "".concat(desc, ": ").concat(damageText, " -- ").concat(koChanceText)
        : "".concat(desc, ": ").concat(damageText);
}
...
function getKOChance(gen, attacker, defender, move, field, damage, err) {
    ...
    function KOChance(chanceWithoutEot, chanceWithEot, n, multipleTurns) {
        ...
        if (chanceWithoutEot === undefined || chanceWithEot === undefined) {
            ...
            if (chanceWithEot === 1) {
                text +=
                    "".concat(roundChance(chanceWithoutEot), "% chance to ").concat(KOTurnText).concat(hazardsText, " ") +
...
function squashMultihit(gen, d, hits, err) {
    ...
    if (d.length === 1) {
        ...
        if (hits > 1) {
            (0, util_1.error)(err, "Unexpected # of hits for Expertise: ".concat(hits));
        }
        var r = [];
        for (var i = 0; i < 16; i++) {
            var val = 0;
